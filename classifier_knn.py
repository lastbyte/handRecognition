...
outfile = open("output/knn_10_output.txt","a")
...
for dim in [10,20,30,40]:
	...
	print clf
	kf = cross_validation.KFold(len(images),n_folds=10, shuffle=True, random_state=4)
...
#		outfile.write(metrics.classification_report(testing_labels, predicted))
	...
	joblib.dump(clf, 'models/knn_10_'+str(dim)+'x'+str(dim)+'.pkl')
	print "Dumped"	
	predicted = clf.predict(testing_images)
	print prec(score)
	outfile.write(prec(clf.score(testing_images, testing_labels))+'\n')
