...
for line in lines:
	...
	cv2.imshow("test_image: "+str(label),frame)
...
#	cv2.imshow("final",sign_image)
	...
	svm_list=[]
	for d in dim:
		knn = joblib.load('models/knn_10_'+str(d)+'x'+str(d)+'.pkl')
		#svm = joblib.load('models/linear_SVC/linearSVC_'+str(d)+'x'+str(d)+'.pkl')

		temp_image = cv2.resize(sign_image,(d,d)).flatten()
		predicted1 = knn.predict(np.array([temp_image]))
